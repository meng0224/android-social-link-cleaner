...
自訂規則不要直接加進 `"providers"` 裡，避免更新官方規則時被覆蓋。

應在最外層、與 `"providers"` 同一層加入 `"自訂規則"`。檔案尾端應改成類似：
...
注意：這只是加到原本規則最外層的片段，不是一份可以獨立取代全部規則的完整 JSON。
